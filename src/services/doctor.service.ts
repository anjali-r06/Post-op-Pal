...
export const doctorService = {
    ...
    const doctors: Doctor[] = [
      {
        ...
        maxPatientCapacity: 25,
        rating: 4.8,
        languages: ['English', 'Hindi'],
        status: 'available',
        bio: 'Specialized in orthopedic surgeries with 15 years of experience.'
      },
      {
        id: 'doc2',
        name: 'Dr. Verma',
  ...
  async assignDoctorToPatient(patientId: string, doctorId: string): Promise<void> {
    console.log(`Assigning doctor ${doctorId} to patient ${patientId}`)
    // In production: await api.post(`/patients/${patientId}/assign-doctor`, { doctorId })
  },

  // Get patient's assigned doctor
  async getPatientDoctor(_patientId: string): Promise<Doctor | null> {
    // In production: const response = await api.get(`/patients/${patientId}/doctor`)
    // return response.data

    // Mock - return assigned doctor
    const doctors = await this.getAllDoctors()
    return doctors.find(d => d.id === 'doc1') || null
  },
}